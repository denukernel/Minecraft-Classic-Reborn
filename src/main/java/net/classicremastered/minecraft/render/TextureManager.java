...
public class TextureManager {
        ...
        if (textures.get(file) != null) {
            ...
            try {
                ...
                if (file.endsWith(".png")) {
                    ...
                    if (file.startsWith("##")) {
                        String inner = file.substring(2);
                        java.io.InputStream in = TextureManager.class.getResourceAsStream(inner);
                        if (in == null) {
                            System.err.println("[TextureManager] Missing texture (##): " + inner);
                            img = getMissingTexture();
                        } else {
                            img = load1(ImageIO.read(in));
                        }
                    } else {
                        java.io.InputStream in = TextureManager.class.getResourceAsStream(file);
                        if (in == null) {
                            System.err.println("[TextureManager] Missing texture: " + file);
                            img = getMissingTexture();
                        } else {
                            img = ImageIO.read(in);
                        }
                    ...
                    if (zip.getEntry(terrainPNG.startsWith("/") ? terrainPNG.substring(1) : terrainPNG) != null) {
                        load(ImageIO.read(zip.getInputStream(zip.getEntry(terrainPNG))), textureID);
                    } else {
                        java.io.InputStream in = TextureManager.class.getResourceAsStream(terrainPNG);
                        if (in == null) {
                            System.err.println("[TextureManager] Missing default terrain.png in " + file);
                            load(getMissingTexture(), textureID);
                        } else {
                            load(ImageIO.read(in), textureID);
                        }
                    }
                    zip.close();
                ...
                System.err.println("[TextureManager] Failed to load: " + file + " (" + e + ")");
                load(getMissingTexture(), idBuffer.get(0));
    ...
    private BufferedImage getMissingTexture() {
        ...
        for (int y = 0; y < 16; y++) {
            for (int x = 0; x < 16; x++) {
                boolean c = ((x < 8) ^ (y < 8));
                g.setColor(c ? Color.MAGENTA : Color.BLACK);
                g.fillRect(x, y, 1, 1);
            }
        }
        g.dispose();
    ...
    public void applyBlockTilesToTerrainAtlas() {
        ...
        try {
            BufferedImage strip = ImageIO
                    .read(TextureManager.class.getResourceAsStream("/terrain/blocks/wooltextures.png"));
            if (strip != null) {
                for (int i = 0; i < 16; i++) {
                    BufferedImage sub = strip.getSubimage(i * (strip.getWidth() / 16), 0, strip.getWidth() / 16,
                            strip.getHeight());
    ...
    public void generateMipMaps(ByteBuffer data, int width, int height, boolean test) {
        ...
        for (int level = test ? 0 : 1; level <= 4; level++) {
            ...
            GL11.glTexImage2D(GL11.GL_TEXTURE_2D, level, GL11.GL_RGBA, mipWidth, mipHeight, 0, GL11.GL_RGBA,
