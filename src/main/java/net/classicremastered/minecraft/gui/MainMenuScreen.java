...
public final class MainMenuScreen extends GuiScreen {
    ...
    public void render(int mouseX, int mouseY) {
        ...
        if (tex != 0) {
            ...
            GL11.glTexCoord2f(0.0F + scroll, 0.0F + scroll); GL11.glVertex2f(0, 0);
            GL11.glTexCoord2f(u + scroll, 0.0F + scroll);    GL11.glVertex2f(this.width, 0);
            GL11.glTexCoord2f(u + scroll, v + scroll);       GL11.glVertex2f(this.width, this.height);
            GL11.glTexCoord2f(0.0F + scroll, v + scroll);    GL11.glVertex2f(0, this.height);
            GL11.glEnd();
        }
     ...
     // --- Classic copyright text ---
        String copy = "Copyright Mojang Specifications. Do not distribute!";
        int copyW = this.fontRenderer.getWidth(copy);
        this.fontRenderer.renderNoShadow(copy,
                this.width - copyW - 2,   // right-align with 2px padding
                this.height - 10,         // 10px up from bottom
                0xFFFFFF);                // white
