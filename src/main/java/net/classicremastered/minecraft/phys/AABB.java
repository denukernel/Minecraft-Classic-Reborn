package net.classicremastered.minecraft.phys;
...
public class AABB implements Serializable {
   ...
   public AABB cloneMove(float dx, float dy, float dz) {
       // Correct: translate every face by (dx, dy, dz)
       return new AABB(this.x0 + dx, this.y0 + dy, this.z0 + dz,
   ...
   public boolean intersects(AABB o) {
	    return o.x1 > this.x0 && o.x0 < this.x1 &&
	           o.y1 > this.y0 && o.y0 < this.y1 &&
	           o.z1 > this.z0 && o.z0 < this.z1;
	}
   ...
   // --- add these methods to AABB ---

   /**
    * Update this AABB in-place (mirror of the constructor).
    * Useful when you want to reuse the same AABB instance instead of allocating.
    */
   ...
   /**
    * Convenience: set AABB from entity position (feet at pos.y) and width/height.
    * width = full width (e.g. 0.6f), height = full height (e.g. 2.9f).
    * Keeps the same convention your Enderman code uses (y = feet).
    */
   public void setFromPosAndSize(float cx, float feetY, float cz, float width, float height) {
      float hw = width * 0.5f;
      this.x0 = cx - hw;
      this.z0 = cz - hw;
      this.x1 = cx + hw;
